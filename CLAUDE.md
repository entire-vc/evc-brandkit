...
```bash
# Tokens package
pnpm --filter @entire-vc/tokens build        # Build tokens (tsup + CSS generation)
pnpm --filter @entire-vc/tokens build:css    # Rebuild theme CSS files only

# UI package (React)
pnpm --filter @entire-vc/ui build            # Build React components
pnpm --filter @entire-vc/ui typecheck        # TypeScript check only

# UI Svelte package
pnpm --filter @entire-vc/ui-svelte build     # Build Svelte components
pnpm --filter @entire-vc/ui-svelte check     # Svelte + TypeScript check

# Docs/Storybook
pnpm --filter @entire-vc/docs build          # Build Storybook for production
```
...
**React projects:**
1. `pnpm add @entire-vc/tokens @entire-vc/ui`
2. Add `presets: [require('@entire-vc/tokens/tailwind-preset')]` to tailwind.config
3. Import theme CSS: `@import '@entire-vc/tokens/css/entire.css'`
4. Add `node_modules/@entire-vc/ui/dist/**/*.{js,mjs}` to Tailwind content paths

**Svelte projects:**
1. `pnpm add @entire-vc/tokens @entire-vc/ui-svelte`
2. Add `presets: [require('@entire-vc/tokens/tailwind-preset')]` to tailwind.config
3. Import theme CSS: `@import '@entire-vc/tokens/css/entire.css'`
4. Add `node_modules/@entire-vc/ui-svelte/dist/**/*.{js,svelte}` to Tailwind content paths
